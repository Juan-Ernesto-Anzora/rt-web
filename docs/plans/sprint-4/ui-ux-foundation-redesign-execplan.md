...
- [x] Diagnose the later docs-only head failure in run `36206811922` and apply a bounded test-only create-to-detail synchronization fix.
- [x] Run the full local gate after that test correction; focused smoke passed five no-retry repetitions and full CI-mode browser smoke passed 48/48 without retries.
- [x] Push test correction `29958d6` and verify hosted `ci-web` [run 36207337094](https://github.com/Juan-Ernesto-Anzora/rt-web/actions/runs/36207337094) passed without flaky cases.
...
- Hosted normal Chromium smoke initially reported 47 passed and one flaky Sprint 3 workflow create/edit case. A later docs-only head [run 36206811922](https://github.com/Juan-Ernesto-Anzora/rt-web/actions/runs/36206811922) failed that same case on both attempts. Its test awaited the create POST request but not the response/UI transition; the following `Name *` assertion could pass against the still-open Create dialog, and the Description edit could target its textarea rather than Workflow detail. Waiting for the dialog to close and scoping Name/Description/Save to the detail form fixes that concrete test race. Five focused no-retry local repetitions pass. No Workflow Admin runtime file changed.
...
- For the separately exposed Admin smoke race, change only the test synchronization/locator scope. Do not increase retries/timeouts or refactor Workflow Admin.
...
The Admin PATCH wait was flaky in the first two hosted runs and failed both attempts on the later documentation-only head. The test's create-request wait did not establish that the Create dialog had closed or the Detail form was active. The bounded test-only synchronization above corrects that race without changing production Admin behavior. After the correction, five focused no-retry repetitions, full CI-mode Chromium smoke (48/48 without retries), theme preview (5/5), Dashboard preview (14/14), typecheck, lint, build, token tests (4) and theme runtime tests (10) pass locally. The production build again used the explicit test API base. The final hosted PR-head result remains pending.

Hosted evidence for CI-fix commit `3795ff8`: PR #28 `ci-web` run [36206349455](https://github.com/Juan-Ernesto-Anzora/rt-web/actions/runs/36206349455) passed in 2m12s. The browser smoke reported 47 passed and the unrelated Admin workflow case flaky/pass-on-retry; theme preview passed 5/5 and Dashboard preview passed 14/14. The follow-up evidence commit `3f5369b` also passed on its own PR-head [run 36206590520](https://github.com/Juan-Ernesto-Anzora/rt-web/actions/runs/36206590520) in 2m9s. The next docs-only [run 36206811922](https://github.com/Juan-Ernesto-Anzora/rt-web/actions/runs/36206811922) failed only in the Admin smoke race before reaching preview. The scoped test correction at `29958d6` then passed hosted [run 36207337094](https://github.com/Juan-Ernesto-Anzora/rt-web/actions/runs/36207337094): 48/48 functional browser cases without flaky/retry, 5/5 theme preview and 14/14 Dashboard preview. GitHub reported PR #28 clean and non-draft at that head. M5D is READY with respect to its required Web CI gate; merging remains a reviewer/maintainer action. M6 remains out of scope; after this closeout, the exact M6 starting point is the independently verified Search/list contract and Search's own fan-out, as documented below.
