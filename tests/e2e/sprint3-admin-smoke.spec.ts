...
async function mockApi(page: Page, authorized = true) {
  ...
  await page.route("**/*", async (route) => {
    ...
    if (path === "/admin/me/permissions/") {
      ...
      return json(route, { tenant_id: "11111111-1111-4111-8111-111111111111", user: { user_id: "22222222-2222-4222-8222-222222222222", display_name: "Admin User", email: "admin@example.com" }, rol: [] });
    }
    ...
    if (path === "/requests/" && request.method() === "POST") { const body = request.postDataJSON(); requestTitle = body.title; return json(route, { request_id: "a0000000-0000-4000-8000-000000000010" }); }
    if (path === "/admin/users/") return json(route, { count: 1, next: null, previous: null, results: [{ user_id: "33333333-3333-4333-8333-333333333333", email: "agent@example.com", display_name: "Agent User" }] });
    ...
    if (path === "/admin/workflows/" && request.method() === "GET") return json(route, workflow ? [workflow] : []);
    if (path === "/admin/workflows/" && request.method() === "POST") { const body = request.postDataJSON(); workflow = { flow_id: "b0000000-0000-4000-8000-000000000001", name: body.name, description: body.description, created_at: new Date().toISOString() }; return json(route, workflow); }
    if (/^\/admin\/workflows\/[0-9a-f-]+\/$/.test(path) && request.method() === "GET") return json(route, { ...(workflow ?? { flow_id: path.split("/")[3], name: "Workflow", description: "" }), statuses: workflowStatuses });
    if (/^\/admin\/workflows\/[0-9a-f-]+\/$/.test(path) && request.method() === "PATCH") { const body = request.postDataJSON(); workflow = { ...(workflow ?? { flow_id: path.split("/")[3], name: "Workflow", description: "" }), ...body, created_at: workflow?.created_at ?? new Date().toISOString() }; return json(route, workflow); }
    if (/^\/admin\/workflows\/[0-9a-f-]+\/statuses\/$/.test(path) && request.method() === "POST") { const body = request.postDataJSON(); const status = { status_id: `d0000000-0000-4000-8000-${String(workflowStatuses.length).padStart(24, "0")}`, flow_id: path.split("/")[3], ...body, created_at: new Date().toISOString() }; workflowStatuses.push(status); return json(route, status); }
    if (path === "/flows/") return json(route, [{ flow_id: "b0000000-0000-4000-8000-000000000001", name: "Browser Demo Flow", description: "" }]);
    if (path === "/flows/b0000000-0000-4000-8000-000000000001/statuses/") return json(route, [{ status_id: "b0000000-0000-4000-8000-000000000002", name: "Open", category: "open", is_terminal: false, created_at: new Date().toISOString() }]);
    if (path === "/users/") return json(route, [{ user_id: "b0000000-0000-4000-8000-000000000003", email: "requester@example.com", display_name: "Requester User" }]);
    if (path === "/reports/summary/") return json(route, { total: 6, open: 2, in_progress: 1, waiting: 0, closed: 3, due_today: 1, overdue: 0, unassigned: 0, assigned_to_me: 2, by_priority: [{ priority: "normal", count: 4 }] });
    if (path === "/admin/settings/") return json(route, { settings: [
      { setting_id: "50000000-0000-4000-8000-000000000001", key: "web_base_url", value: "http://127.0.0.1:5173", value_type: "url", is_sensitive: false, has_value: true, updated_at: "2026-08-01T12:00:00Z" },
      { setting_id: "50000000-0000-4000-8000-000000000002", key: "default_timezone", value: "America/El_Salvador", value_type: "timezone", is_sensitive: false, has_value: true, updated_at: "2026-08-01T12:00:00Z" },
      { setting_id: "50000000-0000-4000-8000-000000000003", key: "default_page_size", value: "25", value_type: "integer", is_sensitive: false, has_value: true, updated_at: "2026-08-01T12:00:00Z" },
      { setting_id: "50000000-0000-4000-8000-000000000004", key: "email_from", value: "rt@example.com", value_type: "email", is_sensitive: false, has_value: true, updated_at: "2026-08-01T12:00:00Z" },
    ] });
    if (path === "/admin/feature-flags/") return json(route, ["adminConsole", "slaEnabled", "exportsEnabled", "notificationTemplates"].map((key, index) => ({ feature_flag_id: `60000000-0000-4000-8${index}-000000000000`, key, enabled: true })));
    if (path === "/admin/notification-templates/") return json(route, ["request.created", "request.assigned", "comment.added", "request.closed"].map((event_type, index) => ({ notification_template_id: `70000000-0000-4000-8${index}-000000000000`, event_type, subject_template: "Template", body_template: "Template" })));
    if (path.startsWith("/admin/notification-templates/") && request.method() === "GET") return json(route, { notification_template_id: path.split("/")[3], event_type: "request.created", subject_template: "Template", body_template: "Template" });
    if (path === "/admin/audit/") return json(route, { count: 1, next: null, previous: null, results: [{ activity_id: "80000000-0000-4000-8000-000000000001", request_id: null, actor_id: null, type: "role", object_type: "role", action: "updated", timestamp: new Date().toISOString() }] });
    if (/^\/requests\/[0-9a-f-]+\/$/.test(path)) return json(route, { request_id: path.split("/")[2], human_id: "RT-2026-000123", title: requestTitle, description: "Request detail smoke data.", priority: "normal", status_id: "b0000000-0000-4000-8000-000000000002", flow_id: "b0000000-0000-4000-8000-000000000001", requester_id: "b0000000-0000-4000-8000-000000000003" });
...
test("workflow create and edit use POST then PATCH", async ({ page }) => {
  ...
  // Fix: Use Promise.all to handle request and action together
...
test("workflow transition sends comment once with exact contract field", async ({ page }) => {
  ...
  await page.route("**/api/requests/a0000000-0000-4000-8000-000000000010/available-transitions/", (route) => 
    json(route, [{ transition_id: "c0000000-0000-4000-8000-000000000001", name: "Move to In Progress" }])
  );
  await page.route("**/api/requests/a0000000-0000-4000-8000-000000000010/transition/", (route) => 
    json(route, { request_id: "a0000000-0000-4000-8000-000000000010", status_id: "c0000000-0000-4000-8000-000000000011" })
  );
  await login(page);
  await page.goto("/requests/a0000000-0000-4000-8000-000000000010");
  
  // Fix: Use Promise.all for button interactions to handle timing properly
