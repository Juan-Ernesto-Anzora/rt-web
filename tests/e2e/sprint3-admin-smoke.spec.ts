...
async function mockApi(page: Page, authorized = true) {
  ...
  await page.route("**/*", async (route) => {
    ...
    if (path === "/requests/" && request.method() === "POST") { const body = request.postDataJSON(); requestTitle = body.title; return json(route, { request_id: "a0000000-0000-4000-8000-000000000010", human_id: "RT-2026-000123", ...body }, 201); }
    ...
    if (path === "/admin/workflows/" && request.method() === "POST") { const body = request.postDataJSON(); workflow = { flow_id: "b0000000-0000-4000-8000-000000000001", name: body.name, description: body.description ?? "", created_at: "2026-08-01T12:00:00Z" }; return json(route, workflow, 201); }
    if (/^\/admin\/workflows\/[0-9a-f-]+\/$/.test(path) && request.method() === "GET") return json(route, { ...(workflow ?? { flow_id: path.split("/")[3], name: "Workflow", description: "" }), statuses: workflowStatuses, transitions: [] });
    if (/^\/admin\/workflows\/[0-9a-f-]+\/$/.test(path) && request.method() === "PATCH") { const body = request.postDataJSON(); workflow = { ...(workflow ?? { flow_id: path.split("/")[3], name: "Workflow", description: "", created_at: "2026-08-01T12:00:00Z" }), ...body }; return json(route, workflow); }
    if (/^\/admin\/workflows\/[0-9a-f-]+\/statuses\/$/.test(path) && request.method() === "POST") { const body = request.postDataJSON(); const status = { status_id: `d0000000-0000-4000-8000-${String(workflowStatuses.length + 1).padStart(12, "0")}`, flow_id: path.split("/")[3], name: body.name, category: body.category, is_terminal: body.is_terminal, created_at: "2026-08-01T12:00:00Z" }; workflowStatuses.push(status); return json(route, status, 201); }
    if (path === "/flows/") return json(route, [{ flow_id: "b0000000-0000-4000-8000-000000000001", name: "Browser Demo Flow", description: "" }]);
    if (path === "/flows/b0000000-0000-4000-8000-000000000001/statuses/") return json(route, [{ status_id: "b0000000-0000-4000-8000-000000000002", name: "Open", category: "open", is_terminal: false }]);
    ...
    if (/^\/requests\/[0-9a-f-]+\/$/.test(path)) return json(route, { request_id: path.split("/")[2], human_id: "RT-2026-000123", title: requestTitle, description: "Request detail smoke data.", priority: "normal", flow: { flow_id: "a0000000-0000-4000-8000-000000000001", name: "IT Support", description: "" }, status: { status_id: "a0000000-0000-4000-8000-000000000002", name: "Open", category: "open", is_terminal: false }, requester: { user_id: "a0000000-0000-4000-8000-000000000003", email: "requester@example.com", display_name: "Requester User" }, assignee: null, due_at: null, created_at: "2026-08-01T12:00:00Z", updated_at: "2026-08-01T12:00:00Z", attachments: [] });
...
test("workflow create and edit use POST then PATCH", async ({ page }) => {
  ...
  const createRequest = page.waitForRequest((request) => request.method() === "POST" && new URL(request.url()).pathname.endsWith("/api/admin/workflows/"));
  await page.getByRole("button", { name: "Create workflow" }).click();
  expect((await createRequest).postDataJSON()).toEqual({ name: "WEB-D10 Workflow", description: "Created in browser smoke" });
  ...
  const patchRequest = page.waitForRequest((request) => request.method() === "PATCH" && /\/api\/admin\/workflows\/[0-9a-f-]+\/$/.test(new URL(request.url()).pathname));
  await page.getByRole("button", { name: "Save workflow" }).click();
  expect((await patchRequest).postDataJSON()).toEqual({ name: "WEB-D10 Workflow", description: "Updated in browser smoke" });
  ...
  const statusRequest = page.waitForRequest((request) => request.method() === "POST" && /\/api\/admin\/workflows\/[0-9a-f-]+\/statuses\/$/.test(new URL(request.url()).pathname));
  await statusForm.getByRole("button", { name: "Add", exact: true }).click();
  expect((await statusRequest).postDataJSON()).toEqual({ name: "Open", category: "open", is_terminal: false });
...
test("request creation uses readable lookups and public IDs", async ({ page }) => {
  ...
  const requestPromise = page.waitForRequest((request) => request.method() === "POST" && new URL(request.url()).pathname.endsWith("/api/requests/"));
...
test("workflow transition sends comment once with exact contract field", async ({ page }) => {
  ...
  await page.route("**/api/requests/a0000000-0000-4000-8000-000000000010/comments/", async (route) => { if (route.request().method() === "POST") duplicateCommentPosts += 1; await json(route, []); });
  await page.route("**/api/requests/a0000000-0000-4000-8000-000000000010/available-transitions/", (route) => json(route, [{ transition_id: "c0000000-0000-4000-8000-000000000001", name: "Move to In Progress", to_status: { status_id: "c0000000-0000-4000-8000-000000000002", name: "In Progress", category: "in_progress", is_terminal: false }, requires_comment: false }]));
  await page.route("**/api/requests/a0000000-0000-4000-8000-000000000010/transition/", (route) => json(route, { request_id: "a0000000-0000-4000-8000-000000000010", status_id: "c0000000-0000-4000-8000-000000000002" }));
  await login(page);
  await page.goto("/requests/a0000000-0000-4000-8000-000000000010");
  ...
  const transitionRequest = page.waitForRequest((request) => request.method() === "POST" && new URL(request.url()).pathname.endsWith("/transition/"));
  await page.getByRole("button", { name: "Apply Action" }).click();
  expect((await transitionRequest).postDataJSON()).toEqual({ transition_id: "c0000000-0000-4000-8000-000000000001", comment: "Single transition comment" });
