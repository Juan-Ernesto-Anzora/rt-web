...
export default function AdminSlaPage() {
  ...
  return <div className="space-y-4">
    ...
    <div className="grid gap-3 rounded-lg border border-neutral-200 bg-white p-4 md:grid-cols-2 xl:grid-cols-[minmax(0,1fr)_180px_180px_auto]">
    ...
    {loading ? <LoadingRows rows={5} /> : policies.length === 0 ? <EmptyState title="No SLA policies" body="No policies match the current filters." /> : <div className="overflow-x-auto rounded-lg border border-neutral-200 bg-white"><table className="w-full min-w-[760px] text-left text-sm"><caption className="sr-only">Tenant SLA policies</caption><thead className="bg-neutral-50 text-neutral-700"><tr><th scope="col" className="px-4 py-3">Policy</th><th scope="col" className="px-4 py-3">Priority</th><th scope="col" className="px-4 py-3">Response</th><th scope="col" className="px-4 py-3">Resolution</th><th scope="col" className="px-4 py-3">State</th><th scope="col" className="px-4 py-3 text-right">Actions</th></tr></thead><tbody className="divide-y divide-neutral-100">{policies.map((policy) => <tr key={policy.id} className="h-12 hover:bg-neutral-50"><td className="px-4 font-semibold text-neutral-900">{policy.name}</td><td className="px-4 capitalize">{policy.priority}</td><td className="px-4">{policy.responseMinutes} min <span className="text-neutral-600">({durationHint(policy.responseMinutes)})</span></td><td className="px-4">{policy.resolutionMinutes} min <span className="text-neutral-600">({durationHint(policy.resolutionMinutes)})</span></td><td className="px-4"><StateBadge label={policy.isActive ? "Active" : "Inactive"} tone={policy.isActive ? "primary" : "neutral"} /></td><td className="px-4 text-right"><div className="flex justify-end gap-2"><button type="button" onClick={() => setEditing(policy)} className="rounded-lg border border-neutral-300 px-3 py-2 font-semibold">Edit</button>{policy.isActive ? <button type="button" onClick={() => setDeactivating(policy)} className="rounded-lg border border-danger-500 px-3 py-2 font-semibold text-danger-500">Deactivate</button> : null}</div></td></tr>)}</tbody></table></div>}
    <PaginationControls page={page} pageSize={25} count={count} label="policies" onPageChange={setPage} />
